...
当前分支正在迁移至平台直采：B站和视频号的望获OS已完成真实后台采集，小红书安芯日记已验证全量笔记、五项累计指标及主站评论回复。其他账号仍需逐个接入，暂未达到全平台上线条件。开发测试请使用 [本地隔离开发说明](docs/LOCAL_DEVELOPMENT.md)，不要直接运行下方生产 Compose。
...
- 四平台评论及二级回复分页框架；B站、视频号、小红书已各验证一个账号，抖音仍待接入。
- 其他平台评论数量增长提醒。
