...
| 浏览器会话、身份校验、响应监听、自动翻页、字段映射 | 已实现；真实 Chromium 本地响应夹具测试通过 | B站、视频号的望获OS已通过；小红书安芯日记内容采集通过、评论待接入 |
...
| 小红书创作后台 | `xiaohongshu_creator` 内置页面配方、容器滚动、首屏总数与置顶去重 | 安芯日记62篇及五项指标通过，主站评论需单独登录后核验 |
| 抖音、知乎、公众号后台 | 可使用统一浏览器执行器和显式字段映射 | 尚需登录确认真实流程 |
