...
| 浏览器会话、身份校验、响应监听、自动翻页、字段映射 | 已实现；真实 Chromium 本地响应夹具测试通过 | B站2账号、抖音2账号、视频号2账号、小红书安芯日记已通过 |
...
视频号“安芯很安心”按用户要求留到实际运行时登录。本地不会将其标为已验证，已有数据保持缓存；运行时使用该账号独立登录入口，再依次执行 `bind-channels`、`export-session`、全量 `probe` 和 `probe-comments`。在完成这些步骤前，不具备该账号的生产验收证据。
