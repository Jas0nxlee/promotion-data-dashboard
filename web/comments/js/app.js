...
function renderKpis(threads) {
  ...
  const cards = [
    ...
    ["今日 API", usage.used ?? 0, `上限 ${usage.limit ?? "--"} 次`, "#f87171"],
  ];
...
function renderBudget() {
  ...
  $("apiBudget").innerHTML = `
    ...
    <div class="usage-breakdown">${tasks.map(([task, count]) => `
      ...
      || `<div class="usage-row"><span>今日尚无 TikHub 调用</span><strong>0</strong></div>`}</div>`;
}
...
function renderUsageChart() {
  ...
  usageChart.setOption({
    ...
    graphic: history.length ? [] : [{ type: "text", left: "center", top: "middle", style: { text: "暂无 API 用量记录", fill: "#8296bd" } }],
  }, true);
}
...
function renderStatus() {
  ...
  status.innerHTML = `<strong>时间线起点：${esc(started)}</strong><span>·</span>
    ...
    ${usageStarted ? `<span>· API 记账自 ${esc(shortTime(usageStarted))} 开始</span>` : ""}`;
}
